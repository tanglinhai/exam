...
const operations = [{name:'专家评审-合理低价法'},{name:'专家评审-综合评分办法'},{name:'专家评审-双信封评标办法'}];

Operation.find(function(err, docs){
  if(!docs || docs.length == 0)
  for(var i=0;i<operations.length;i++){
    Operation.create(operations[i]);
  }
});
        ...
        Paper.create(paperData,function (err1,doc1) {
          if (err1) {
            ...
            if (doc1) {
              ...
              doc.age = Math.random()*1000;
              Teacher.updateOne({_id:doc._id}, doc,function(err){
                if(err){
                  console.log(err);
                }else{
