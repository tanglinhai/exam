<template>
    <div class="fronttop">
      ...
      <div class="top-user">
        <ul class="user-info">
          <li>欢迎！{{userData.userName}}</li>
          <li>工号: {{userData.userId}}</li>
          <li>级别: {{userData.grade}} 级</li>
          <li>专业类别: {{userData.class}} </li>
        </ul>
        <el-button type="danger" size="mini" class="login-out" @click="loginOut">退出</el-button>
        <router-link class="person" :to="{name:'HelpCenter'}">帮助中心</router-link>
        <router-link class="person" :to="{name:'Simulationpractice'}">模拟练习</router-link>
