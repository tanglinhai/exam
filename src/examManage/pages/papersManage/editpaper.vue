	...
	<div class="add">
      ...
      <div class="question_list">
        ...
        <ul >
          ...
          <li>
            <el-button type="primary" class="button" @click="addNewFormItem('operation')">操作题</el-button>
          </li>
        </ul>
      </div>
      ...
      <el-dialog  :title="isEdit?'修改问题':'新增问题'" :visible.sync="dialogVisible" size="tiny" class="form_item_dialog" @close="reloadDialog">
        <el-form :model="dialogForm" ref="dialogForm"  :rules="dialogRules" label-width="72px">
          <el-form-item label="题目：" prop="name">
            <el-input placeholder="请输入题目" v-model="dialogForm.name" :disabled="dialogForm.type=='operation'"></el-input>
          </el-form-item>
          ...
          <div  v-if="dialogForm.type!='judgement'&&dialogForm.type!='Q&A' && dialogForm.type!='operation'">
          ...
          <el-form-item label="答案：" prop="answer" v-if="dialogForm.type!='Q&A' && dialogForm.type!='operation'">
          ...
          <div v-if="dialogForm.type=='operation'">
            <el-form-item label="操作题库：" prop="_operation">
              <el-radio-group v-model="dialogForm._operation" size="medium" @change="setOperationTitle(dialogForm._operation)">
                <el-radio :label="o._id" v-for="o in operations">{{o.name}}</el-radio>
              </el-radio-group>
            </el-form-item>
          </div>
        </el-form>
        ...
        <ul>
          <li v-for="(item,index) in form._questions" :key="item.id" class="marginB10">
            <p class="question-title">{{index+1}}、({{item.type=='single'?'单选题'
              :item.type=='multi'?'多选题':item.type=='judgement'?'判断题':item.type=='Q&A'?'简答题':'操作题'}}){{item.name}} ({{item.score}}分)
...
export default {
	data() {
      ...
      dialogRules:{
        ...
        operation:[
          { type:'string', required: true, message: '请选择一个操作题目', trigger: 'change' },
        ]
      },
      operations:[],
      // 弹窗表单
	...
	methods: {
    setOperationTitle(operationId){
      for(var i=0;i<this.operations.length;i++){
        if(this.operations[i]._id == operationId){
          this.dialogForm.name = this.operations[i].name;
          return;
        }
      }
    },
